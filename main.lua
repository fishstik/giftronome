...
function love.draw()
	...
	--draw text shadow
		...
		-- gfx.print(string.format("%.1f bpm", bpm), 0+offset, 0+offset)

		-- if inputMode_artist then gfx.setColor(red) end
		-- if isUrlTempo then gfx.setColor(cyan) end
		-- if not utf8_validator.validate(artist) then artist = "ERROR: NOT VALID UTF-8" end
		-- gfx.print(string.format("artist: [%s]", artist), 0+offset, 2*font_size+offset)

		-- gfx.setColor(maincolor)
		-- if inputMode_song then gfx.setColor(red) end
		-- if isUrlTempo then gfx.setColor(cyan) end
		-- if not utf8_validator.validate(song) then song = "ERROR: NOT VALID UTF-8" end
		-- gfx.print(string.format("song: [%s]", song), 0+offset, 3*font_size+offset)

		-- gfx.setColor(maincolor)

		-- gfx.print("H: show/hide controls", 0+offset, 5*font_size+offset)
		-- if (showHelp) then
		-- 	gfx.print("esc: exit", 					0+offset, 6*font_size+offset)
		-- 	gfx.print("[/]: adjust BPM", 			0+offset, 7*font_size+offset)
		-- 	gfx.print("</>: halve/double BPM", 		0+offset, 8*font_size+offset)
		-- 	gfx.print("space: tap BPM", 			0+offset, 9*font_size+offset)
		-- 	gfx.print("{/}: adjust countdown", 		0+offset, 10*font_size+offset)
		-- 	gfx.print("R: jump to frame 1", 		0+offset, 11*font_size+offset)
		-- 	gfx.print("up/down: zoom",				0+offset, 12*font_size+offset)
		-- 	gfx.print("left/right: switch GIFs",	0+offset, 13*font_size+offset)
		-- 	gfx.print("enter: input artist & song",	0+offset, 14*font_size+offset)
		-- 	gfx.print("L: input last.fm user", 		0+offset, 15*font_size+offset)
		-- 	gfx.print("W: fetch last.fm playing",	0+offset, 16*font_size+offset)
		-- 	gfx.print("A/D: seek WPM", 				0+offset, 17*font_size+offset)
		-- 	gfx.print("S: play/pause WMP", 			0+offset, 18*font_size+offset)
			
		-- end

		-- if inputMode_lfm then gfx.setColor(red) end
		-- gfx.print(string.format("last.fm user: [%s]", lfmUser), offset, sheight-10-3*font_size+offset)
		...
		-- gfx.print(string.format("next fetch in %d:%02d", math.floor(lfmcountdownprint/60), math.floor(lfmcountdownprint % 60)), 0+offset, sheight-10-2*font_size+offset)
		-- gfx.print(string.format("%d/%d: %s", select, #folders, files[select][n]), 0+offset, sheight-10-1*font_size+offset)
